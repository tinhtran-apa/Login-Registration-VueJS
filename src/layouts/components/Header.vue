    ...
    >
      <aside v-show="menuMobile" class="absolute h-screen w-full bg-transparent z-10 block sm:hidden">
        <div class="flex flex-col max-w-[220px] p-5 bg-primary h-screen gap-5">
          <template v-for="head in heads" :key="head">
            <RouterLink :to="head.path" :class="checkPathMobile(head.path)">{{ head.title }}</RouterLink>
          </template>
        </div>
      </aside>
    </Transition>
...
const checkPathDesktop = (field) => {
  return [
    ...
    route.path == field ? "sm:rounded-full px-3 py-1 bg-tertiary text-primary" : "",
  ];
};

const checkPathMobile = (field) => {
  return [
